...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, mask=None):
        # TBD: global visual input in RN backbone
        def stem(x):
        ...
        x4 = self.layer4(x3)
        if mask is not None:
            mask_resized = torch.tensor(mask).unsqueeze(1).float().cuda()
            mask_resized = F.interpolate(mask_resized, size=(7, 7), mode='nearest')
            mask_resized = mask_resized.squeeze(1)
            x4 = x4 * mask_resized.unsqueeze(1)
            #x4 = x4 / x4.norm(dim=1, keepdim=True) # normalize
        x4 = self.attnpool(x4)
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for resblock in self.resblocks:
            x = resblock(x)
            feature_list.append(x.permute(1, 0, 2)) # NLD
...
class TransformerWithMask(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        for i, resblock in enumerate(self.resblocks):
            x = resblock(x)
            if i >=9 and mask is not None:
                x = x.permute(1,0,2)
                mask_resized = torch.tensor(mask).unsqueeze(1).float().cuda()
                mask_resized = F.interpolate(mask_resized, size=(7, 7), mode='nearest')
                mask_resized = mask_resized.squeeze(1)
                x_to_be_masked = einops.rearrange(x[:,1:,:], 'b (h w) c -> b h w c', h=7)
                x_masked = x_to_be_masked * mask_resized.unsqueeze(-1)
                x_masked = einops.rearrange(x_masked, 'b h w c -> b (h w) c')
                x[:, 1:, :] = x_masked
                x = x.permute(1,0,2)
            feature_list.append(x)
        return x, feature_list #self.resblocks(x)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int,
        ...
        self.transformer = TransformerWithMask(width, layers, heads)
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        x, feature_list = self.transformer(x,mask)
